...
for index, row in nwd.iterrows():    #Se recorre la matriz
  ...
  for element in row[1:]:            #aqui inicia desde el 1 para que no divida la escala
    try:
      ...
      resultado[index].append((element/(row[0]-1))+1)   # Se toma la escala (row[0]) y se le resta 1 (para que sea valor par)
                                                        ...
                                                        #Quedando en la misma columna cada uno de los valores de los criterios
    except:
